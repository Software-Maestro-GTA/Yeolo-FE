 ...
 * @description Unit test for updatePreferencesApi (API-PREF-1) in @yeolo/common.
 */
import { updatePreferencesApi, ApiError } from '@yeolo/common';
  ...
  it('MBTI 값이 전송되었을 때 PATCH /api/users/me/preferences 200 OK 성공 응답을 반환해야 한다', async () => {
    fetchSpy = jest
      ...
      .mockImplementation(async (input: any, init: any) => {
        ...
        expect(method?.toUpperCase()).toBe('PATCH');
