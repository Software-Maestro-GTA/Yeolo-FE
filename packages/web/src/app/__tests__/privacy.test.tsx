...
describe('PrivacyPolicyPage Component', () => {
  ...
  it('renders all privacy policy articles from 1 to 6 and appendix', () => {
    ...
    expect(
      screen.getByText('제1조 (수집하는 개인정보 항목 및 수집방법)'),
    ).toBeInTheDocument();
    expect(
      screen.getByText('제2조 (개인정보의 수집 및 이용 목적)'),
    ).toBeInTheDocument();
    expect(
      screen.getByText('제3조 (개인정보의 보유 및 이용 기간)'),
    ).toBeInTheDocument();
    expect(
      screen.getByText('제4조 (개인정보의 제3자 제공 및 처리위탁)'),
    ).toBeInTheDocument();
    expect(
      screen.getByText('제5조 (이용자 및 법정대리인의 권리와 그 행사방법)'),
    ).toBeInTheDocument();
    expect(
      screen.getByText('제6조 (개인정보 보호책임자 및 문의처)'),
    ).toBeInTheDocument();
