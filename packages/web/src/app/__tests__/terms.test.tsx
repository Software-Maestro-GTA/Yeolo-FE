...
describe('TermsOfServicePage Component', () => {
  ...
  it('renders all terms articles from 1 to 8 and appendix', () => {
    ...
    expect(screen.getByText('제4조 (서비스의 제공 및 변경)')).toBeInTheDocument();
    expect(screen.getByText('제5조 (이용자의 의무 및 금지사항)')).toBeInTheDocument();
    expect(screen.getByText('제6조 (계약 해지 및 이용 제한)')).toBeInTheDocument();
    expect(screen.getByText('제7조 (면책 조항)')).toBeInTheDocument();
    expect(screen.getByText('제8조 (분쟁의 해결 및 관할 법원)')).toBeInTheDocument();
